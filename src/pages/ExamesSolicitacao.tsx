...
export default function ExamesSolicitacao() {
  ...
  function handlePrint() {
    ...
    if (!examsText.trim()) { toast.error("Adicione ao menos um exame."); return; }
  ...
  return (
    <div className="animate-fade-in">
      <div className="no-print">
        ...
        <p className="text-sm text-gray-500 mb-6">Selecione os perfis ou adicione exames manualmente.</p>
          ...
          <div key={via} className="print-page-half">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-xs mb-2">
              <div className="col-span-2">
                ...
                {patientSex && <span> · Sexo: {patientSex === "M" ? "( X ) Masculino  (   ) Feminino" : "(   ) Masculino  ( X ) Feminino"}</span>}
              </div>
              {patientDOB && <div><span className="font-semibold">Data de nascimento:</span> {isoToBR(patientDOB)}</div>}
              {motherName && <div className="col-span-2"><span className="font-semibold">Nome da mãe:</span> {motherName}</div>}
            </div>
